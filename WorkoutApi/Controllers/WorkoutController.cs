...
public class WorkoutController : ControllerBase
{
    ...
    public async Task<ActionResult<List<SavedWorkout>>> GetUserWorkouts()
    {
        ...
        if (currentUserId == null)
            return BadRequest("Username not found in authentication");
    ...
    public async Task<IActionResult> CreateWorkout(SavedWorkoutViewModel vm)
    {
        var currentUserId = User.FindFirst(c => c.Type == ClaimTypes.NameIdentifier)?.Value;
        if (currentUserId == null)
            return BadRequest("User name not found in authentication");

        var user = await _client
            .From<DbUser>()
            .Filter(u => u.Id.ToString(), Constants.Operator.Equals, currentUserId)
            .Single();
        if (user == null)
            return BadRequest();
    ...
    public async Task<ActionResult<SavedWorkout>> GetWorkout(int workoutId)
    {
        ...
        if (currentUserId != workout.User.Id.ToString())
            return Unauthorized("User ID does not match workout User ID");
    ...
    [Authorize]
    public async Task<ActionResult<SavedWorkoutViewModel>> EditWorkout(
        int workoutId,
        SavedWorkoutViewModel vm
    )
    {
        // Authentication
        var currentUserId = User.FindFirst(c => c.Type == ClaimTypes.NameIdentifier)?.Value;
        ...
        if (existing == null)
            return NotFound();
        // Authorization
        if (existing.User.Id.ToString() != currentUserId)
    ...
    public async Task<IActionResult> DeleteWorkout(int workoutId)
    {
        ...
        await _client.From<SavedWorkout>().Where(w => w.Id == workoutId).Delete();
